...
`/accounts/me` 호출 전략을 매 네비게이션 → 하이브리드(TTL + visibility + 401 lazy)로 변경하면서 드러난/생긴 작업 항목들이다. 우선순위는 ★ (긴급) ~ ☆ (선택).

## ★ P1. ProtectedRoute: hasAlertedRef로 인한 인증 만료 알림 누락
...
## ★ P1. ProtectedRoute: `alert()` 사용 → react-hot-toast로 교체
...
- 그에 맞춰 `hasAlertedRef` 정책 재검토 (P1 이슈와 함께)

---

## ★ P2. AuthProvider 단위 테스트 부재
...
## ☆ P3. Safari/iOS bfcache 복귀 시 세션 재확인 누락
...
## ☆ P3. TTL_MS (5분) 매직 넘버 → 환경설수화 또는 백엔드 캐시 힌트 사용
...
## ☆ P3. httpClient 모듈 전역 unauthorizedHandler 메모리 누수 가능성
...
## ☆ P4. Pre-existing: App.tsx react-router-dom v7 `<Routes key>` 컴파일 에러
...
## ☆ P4. Pre-existing: config/env.ts의 `import.meta.env` 타입 선언 누락
